...
def crop_gii(gii_path,num_TR,outpath, extension = '.func.gii'):
    ...
    crop_gii_path = os.path.join(outpath,os.path.split(gii_path)[-1].replace(extension,'_cropped'+extension))
...
def highpass_gii(filename,polyorder,deriv,window,outpth, extension = '.func.gii'):
    ...
    if not os.path.isfile(filename): # check if file exists
        ...
        filepath_sg = os.path.join(outpth,os.path.split(filename)[-1].replace(extension,'_sg'+extension))
...
def psc_gii(gii_file,outpth, method='median', extension = '.func.gii'):
    ...
    if not os.path.isfile(gii_file): # check if file exists
        ...
        new_name =  os.path.split(gii_file)[-1].replace(extension,'_psc'+extension) # file name
    ...
    return np.array(psc_gii),psc_gii_pth
...
def smooth_gii(gii_file, outdir, fwhm = 5, extension = '.func.gii'):
    ...
    return np.array(smooth_gii),smooth_gii_pth
