...
# run for all subjects selected
for ind,s in enumerate(sj):
    
    postfmriprep_pth = os.path.join(deriv_pth,'post_fmriprep','soma',s) # path to post_fmriprep files

    out_pth = os.path.join(deriv_pth,'soma_fit',s) # path to save estimates
    ...
    events_dir = [run for run in glob.glob(os.path.join(sourcedata_pth,s,'*','func/*')) if 'soma' in run and 'median' not in run and run.endswith('events.tsv')]
    ...
    for j,lbl in enumerate(bhand_label):
        ...
        # mask data - only significant voxels for region
        if lbl == 'lhand':
            z_masked = mask_arr(stats_all['RL_hand']['zscore'],
                        threshold = z_threshold, side = 'below')

            data_masked = data_masked_all['upper_limb'].copy()
            data_masked[np.isnan(z_masked)] = np.nan
        ...
        #list of all fingers in one hand  
        hand_label = [s for s in params['fitting']['soma']['all_contrasts']['upper_limb'] if lbl in s] 
        # list of lists with other fingers to contrast 
        otherfings = leave_one_out(hand_label) 

        for i,fing in enumerate(hand_label):

            contrast = set_contrast(design_matrix.columns,[[fing],otherfings[i]],[1,-1/4.0],num_cond=2)

            # save estimates in dir 
            stats_filename = os.path.join(out_pth,'glm_stats_%s_vs_all_contrast_zscore_thresh-%.2f.npz' %(fing,z_threshold))
            ...
            if not os.path.isfile(stats_filename): # if doesn't exist already
                ...
                print('%s already exists'%stats_filename)
    ...
    # list of lists with other fingers to contrast 
    otherface = leave_one_out(params['fitting']['soma']['all_contrasts']['face']) 

    for i,part in enumerate(params['fitting']['soma']['all_contrasts']['face']):

        contrast = set_contrast(design_matrix.columns,[[part],otherface[i]],[1,-1/3.0],num_cond=2)

        # save estimates in dir 
        stats_filename = os.path.join(out_pth,'glm_stats_%s_vs_all_contrast_zscore_thresh-%.2f.npz' %(part,z_threshold))

        if not os.path.isfile(stats_filename): # if doesn't exist already

            soma_stats = Parallel(n_jobs=16)(delayed(compute_stats)(vert, design_matrix.values,contrast,betas[w]) for w,vert in enumerate(data_masked_all['face']))
            soma_stats = np.vstack(soma_stats) # t_val,p_val,zscore
